...
| 最小击退距离 | 5像素     | 确保最小击退效果 |
| 最大击退距离 | 50像素    | 限制最大击退距离 |
| 击退速度     | 50像素/秒 | 击退移动速度     |

### 击退触发条件
1. **近战攻击**: 所有近战单位攻击时自动触发击退
2. **攻击成功**: 必须造成实际伤害才能触发击退
3. **目标存活**: 目标单位必须存活才能被击退
4. **距离限制**: 击退距离不超过攻击者的攻击范围
...
**击退计算步骤**:
1. 基础击退距离: 15像素
2. 体型差系数: 攻击者体型 ÷ 目标体型 (最大2倍)
3. 目标击退抗性: 根据体型获取抗性值
4. 攻击类型修正: 根据攻击类型获取修正值
5. 伤害修正: 攻击伤害 ÷ 20 (最大1.5倍)
6. 最终击退距离: (基础距离 × 体型系数 × 攻击修正 × 伤害修正) ÷ 目标抗性
7. 限制击退距离: 确保在5-50像素范围内
...
| 攻击者体型  | 目标体型    | 体型比 | 击退系数 | 击退距离(基础15) | 实际效果 |
| ----------- | ----------- | ------ | -------- | ---------------- | -------- |
| 12 (哥布林) | 35 (骨龙)   | 0.34   | 0.34     | 5.1像素          | 轻微击退 |
| 15 (小恶魔) | 20 (石像鬼) | 0.75   | 0.75     | 11.25像素        | 中等击退 |
| 20 (石像鬼) | 15 (小恶魔) | 1.33   | 1.33     | 19.95像素        | 强击退   |
| 35 (骨龙)   | 12 (哥布林) | 2.92   | 2.0      | 30.0像素         | 极强击退 |
| 30 (石魔像) | 30 (石魔像) | 1.0    | 1.0      | 15.0像素         | 标准击退 |
